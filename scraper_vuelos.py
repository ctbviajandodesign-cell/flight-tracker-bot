...
def format_date(date_str, fallback_day):
    ...
    if len(parts) == 2: return f"{parts[0]}-{parts[1]}-{fallback_day}"
    elif len(parts) == 3:
        if len(parts[0]) == 4: return date_str
        elif len(parts[2]) == 4: return f"{parts[2]}-{parts[1]}-{parts[0]}"
    return None
...
async def procesar_rutas():
    ...
    try:
        ...
        for row in reader:
            ...
            if origen and destino:
                rutas.append({"origen": origen, "destino": destino, "inicio": format_date(mes_ini, "01"), "fin": format_date(mes_fin, "28"), "alerta": alerta, "dias_paquete": paquete})
    except: return []
    ...
    async with async_playwright() as p:
        ...
        for r in rutas:
            ...
            if res:
                resultados.append({"ruta": f"{r['origen']} ➡️ {r['destino']}", "mejores": res['mejores'], "precio": res['precio'], "url": res['url'], "alerta_manual": r['alerta'], "mediana": res['mediana'], "es_ganga_mat": res['es_ganga_mat']})
        await browser.close()
    return resultados
